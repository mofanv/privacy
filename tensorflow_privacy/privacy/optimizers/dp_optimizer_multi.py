...
def make_optimizer_class(cls):
    ...
    class DPOptimizerClass(cls):
        ...
        def __init__(
            ...
            self._dp_nested_query = dp_nested_query
            self._num_microbatches = num_microbatches
            self._global_state = self._dp_nested_query.initial_global_state()
        ...
        def compute_gradients(self,
            ...
            if callable(loss):
                ...
                sample_state = self._dp_nested_query.initial_sample_state(
                    var_list)
                microbatches_losses = tf.reshape(vector_loss,
                                                 [self._num_microbatches, -1])
                sample_params = (
                    self._dp_nested_query.derive_sample_params(self._global_state))
                ...
                def process_microbatch(i, sample_state):
                    ...
                    sample_state = self._dp_nested_query.accumulate_record(
                ...
                grad_sums, self._global_state = (
                    self._dp_nested_query.get_noised_result(
                ...
                microbatches_losses = tf.reshape(
                    loss, [self._num_microbatches, -1])
                sample_params = (
                    self._dp_nested_query.derive_sample_params(self._global_state))
                ...
                if self._unroll_microbatches:
                    ...
                    def cond_fn(i, _): return tf.less(
                        i, self._num_microbatches)
                    def body_fn(i, state): return [tf.add(i, 1), process_microbatch(
                        i, state)]  # pylint: disable=line-too-long
                    idx = tf.constant(0)
...
def make_multi_gaussian_optimizer_class(cls):
    """Constructs a DP optimizer with Gaussian averaging of updates."""

    class DPMultiGaussianOptimizerClass(make_optimizer_class(cls)):
        """DP subclass of given class cls using Gaussian multiple."""
        ...
        def __init__(
            ...
            query1 = gaussian_query.GaussianAverageQuery(
                l2_norm_clip[0], l2_norm_clip[0] * noise_multiplier[0], scalars[0])
            query2 = gaussian_query.GaussianAverageQuery(
                l2_norm_clip[1], l2_norm_clip[1] * noise_multiplier[1], scalars[1])
            query3 = gaussian_query.GaussianAverageQuery(
                l2_norm_clip[2], l2_norm_clip[2] * noise_multiplier[2], scalars[2])
            ...
            query5 = gaussian_query.GaussianAverageQuery(
                l2_norm_clip[4], l2_norm_clip[4] * noise_multiplier[4], scalars[4])
            query6 = gaussian_query.GaussianAverageQuery(
                l2_norm_clip[5], l2_norm_clip[5] * noise_multiplier[5], scalars[5])
            query7 = gaussian_query.GaussianAverageQuery(
                l2_norm_clip[6], l2_norm_clip[6] * noise_multiplier[6], scalars[6])
            query8 = gaussian_query.GaussianAverageQuery(
                l2_norm_clip[7], l2_norm_clip[7] * noise_multiplier[7], scalars[7])
            query9 = gaussian_query.GaussianAverageQuery(
                l2_norm_clip[8], l2_norm_clip[8] * noise_multiplier[8], scalars[8])
            query10 = gaussian_query.GaussianAverageQuery(
                l2_norm_clip[9], l2_norm_clip[9] * noise_multiplier[9], scalars[9])
            query11 = gaussian_query.GaussianAverageQuery(
                l2_norm_clip[10], l2_norm_clip[10] * noise_multiplier[10], scalars[10])
            query12 = gaussian_query.GaussianAverageQuery(
                l2_norm_clip[11], l2_norm_clip[11] * noise_multiplier[11], scalars[11])
            ...
            if ledger:
                dp_nested_query = privacy_ledger.QueryWithLedger(dp_nested_query,
                                                                 ledger=ledger)

            super(DPMultiGaussianOptimizerClass, self).__init__(
                dp_nested_query,
                num_microbatches,
                unroll_microbatches,
                *args,
    ...
    return DPMultiGaussianOptimizerClass
...
DPGradientDescentMultiGaussianOptimizer = make_multi_gaussian_optimizer_class(
